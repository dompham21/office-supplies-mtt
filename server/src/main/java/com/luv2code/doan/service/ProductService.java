...
//        return productAfterSave;
    ...
    public Product addProduct(ProductRequest body, Staff staff) throws DuplicateException, BrandNotFoundException, CategoryNotFoundException, SupplierNotFoundException {
        Category category = categoryService.getCategoryById(body.getCategoryId());
        if(this.existsByName(body.getName())) {
            throw new DuplicateException("Tên sản phẩm không được trùng");
        }
...
//        priceHistory.setStaff(staff);
    ...
    @Transactional
    public Product addProductSeller(ProductRequest body, Seller seller) throws DuplicateException, CategoryNotFoundException {
        Category category = categoryService.getCategoryById(body.getCategoryId());
    ...
    public Page<Product> getListProductsSeller( int pageNo, int pageSize, String sortField, String sortDirection, String keyword, Seller seller) {
        Sort sort = sortDirection.equalsIgnoreCase(Sort.Direction.ASC.name())
                ? Sort.by(sortField).ascending() : Sort.by(sortField).descending() ;
        Pageable pageable = PageRequest.of(pageNo -1, pageSize,sort);
        if(keyword != null) {
            return productRepository.getListProductsSellerWithKeyword(keyword, seller.getId(), pageable);
        }
        else {
            return productRepository.getListProductsSeller(seller.getId(), pageable);
        }
    }
