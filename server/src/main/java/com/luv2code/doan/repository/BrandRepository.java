...
public interface BrandRepository extends JpaRepository<Brand, String> {
    ...
    @Query("SELECT c FROM Brand c WHERE (c.name LIKE %:keyword% OR c.description LIKE %:keyword%)")
    public Page<Brand> getListBrandsAdminWithKeyword(String keyword, Pageable pageable);

    @Query("SELECT c FROM Brand c")
    public Page<Brand> getListBrandsAdmin(Pageable pageable);
