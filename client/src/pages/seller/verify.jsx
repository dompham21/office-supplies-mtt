...
export default function VerifyPage() {
  ...
  const { mutate: verify, isLoading: loading } = useVerifySellerMutation();
  const { mutate: resend, isLoading: loadingResend } = useResendSellerMutation();
...
const [otp, username] = watch(["otp", "username"]);
  ...
  useEffect(() => {
    setValueEmail(localStorage.getItem("email_verify") || "")
    setValue("username", localStorage.getItem("email_verify") || "")
  }, [])
  ...
  
        {
          onSuccess: (value) => {
            ...
            if (response) {
              ...
              if (result == 1) {
                toast.success("Xác thực thành công, vui lòng chờ người quản lý duyệt!", {
                  position: 'top-right',
                  autoClose: 5000,
