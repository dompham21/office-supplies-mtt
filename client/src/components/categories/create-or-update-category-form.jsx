...
import { useUpdateCategoryMutation } from "@data/category/admin/use-update-category.mutation";
import { useCreateCategoryMutation } from "@data/category/admin/use-create-category.mutation";
import { toast } from "react-toastify";

const categoryValidationSchema = yup.object().shape({
  id: yup.string().required("Vui lòng nhập mã danh mục"),
...
export default function CreateOrUpdateCategoryForm({ initialValues }) {
  ...
  const onSubmit = async (values) => {
    ...
    if(initialValues) { // updating
      ...
      },
      {
        ...
        onSettled: () => {
            queryClient.invalidateQueries({ queryKey:[API_ENDPOINTS.CATEGORIES_ADMIN, initialValues?.id?.toString()]});
        },            
      });
    }
    else {
      ...
      },
      {
        onSuccess: ( value ) => {
            ...
            if (response) {
                ...
                if(result == 1) {
                  toast.success('Thêm thông tin danh mục thành công!', {
                    position: "top-right",
                    autoClose: 5000,
                    hideProgressBar: false,
                    closeOnClick: true,
              ...
              toast.error('Thêm thông tin danh mục thất bại!', {
                position: "top-right",
                autoClose: 5000,
                hideProgressBar: false,
                closeOnClick: true,
       ...
       <FormProvider {...methods}>
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          <div className="flex flex-wrap pb-8 border-b border-dashed border-border-base my-5 sm:my-8">
            <Description
              title={"Description"}
              details={"Edit your category description and necessary information from here"}
          ...
          <div className="flex flex-wrap pb-8 border-b border-dashed border-border-base my-5 sm:my-8">
            <Description
              ...
              details={"Upload your category featured image here"}
              className="w-full px-0 sm:pe-4 md:pe-5 pb-5 sm:w-4/12 md:w-1/4 sm:py-8"
            />
