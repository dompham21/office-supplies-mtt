...
const registrationFormSchema = yup.object().shape({
  name: yup.string().required("Vui lòng nhập tên người dùng!").trim()
  .matches(/^[^\`\-\!\[\]\{\}\"\'\>\<\%\^\*\?\/\\\|\,\;\:\+\=\(\)\@\$\&\!\.\#\_0-9]*$/g, "Tên không bao gồm ký tự đặc biệt và số!"),
  email: yup
    .string()
    .email("Sai định dạng email!")
    .required("Vui lòng nhập địa chỉ email!").trim(),
  password: yup.string().required("Vui lòng nhập mật khẩu người dùng!").trim(),
  phone: yup.string().required("Vui lòng nhập số điện thoại người dùng!").trim()
  .matches(/(0[3|5|7|8|9])+([0-9]{8})\b/g, "Không đúng định dạng số điện thoại Việt Nam!"),
});
