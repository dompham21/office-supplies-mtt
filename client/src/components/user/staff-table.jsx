...
import ViewStaffDetail from './view-staff-detail';

function StaffTable({ users, totalPage, pageSize, pageNo, sortField, sortDirection, keyword, loading, handleTableChange }) {
    ...
    const { mutate: deleteUser, isLoading: loadingDelete } = useDeleteStaffMutation();
    const { mutate: approveUser, isLoading: loadingApprove } = useApproveStaffMutation();


    const handleDeleteUser = (e, id) => {
        e.preventDefault()
        confirm({
            title: `Bạn có chắc chắn muốn vô hiệu hoá tài khoản với mã nhân viên ${id} không?`,
                    ...
                    },
                    {
                      onSuccess: ( value ) => {
                          ...
                          if (response) {
                              ...
                              if(result == 1) {
                                toast.success('Vô hiệu hoá tài khoản nhân viên thành công!', {
                                  position: "top-right",
                                  autoClose: 5000,
                            ...
                            toast.error('Vô hiệu hoá tài khoản nhân viên thất bại!', {
                              position: "top-right",
                              autoClose: 5000,
                              hideProgressBar: false,
                              closeOnClick: true,
                      ...
                      onSettled: () => {
                        queryClient.invalidateQueries({ queryKey: [API_ENDPOINTS.STAFFS_ADMIN, {
                            "pageSize": 10,
                            "sortField": sortField,
    ...
    const handleApproveUser = (e, id) => {
        e.preventDefault()
        confirm({
            title: `Bạn có chắc chắn muốn kích hoạt tài khoản với mã nhân viên ${id} không?`,
            icon: <ExclamationCircleFilled />,
            okText: "Kích hoạt",
            cancelText: "Huỷ bỏ",
            onOk() {
                    ...
                    },
                    {
                      onSuccess: ( value ) => {
                          ...
                          if (response) {
                            ...
                            toast.error('Kích hoạt tài khoản nhân viên thất bại!', {
                              position: "top-right",
                              autoClose: 5000,
                              hideProgressBar: false,
                              closeOnClick: true,
    ...
    let columns = [
      ...
      },
      {
        ...
        render: (id, record) => (
          <ActionButtons
            id={record?.id}
            editUrl={`${router.asPath}/${id?.trim()}/edit`}
  ...
  return (
    <Fragment>
      ...
      currentId && isModalOpen && 
        <ViewStaffDetail id={currentId} open={isModalOpen} setIsModalOpen={setIsModalOpen}/>
      }
